...
```{r}
adipose_data1 <- adipose_data %>% select(!contains("date"))

imputed_Data <- mice(adipose_data1, m=5, maxit = 50, seed = 500)
summary(imputed_Data)
imputed_Data$imp$debulking_status
...
# set.seed(1234)
mice(adipose_data1$debulking_status, method = "polyreg")
imp <- mice(adipose_data1[1:5])
imp
imp$imp

imp1 <- mice(adipose_data1[1:5], 
             formulas = list(weight ~ 0 + id + mrn + weight_unit + weight_comment)
             )
```
