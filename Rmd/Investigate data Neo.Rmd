...
tbl2 <- 
  coxph(Surv(time = adipose_data$recurrence_time,
             event = adipose_data$rec_event) ~ raceeth1 + age_at_diagnosis + tnm_cs_mixed_group_stage + debulking_status + bmi_cat, data =  adipose_data) %>%
...
tbl2 <- 
  coxph(Surv(time = adipose_data$os_time,
             event = adipose_data$os_event) ~ raceeth1 + age_at_diagnosis + tnm_cs_mixed_group_stage + debulking_status + bmi_cat, data =  adipose_data) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
```
<br>
