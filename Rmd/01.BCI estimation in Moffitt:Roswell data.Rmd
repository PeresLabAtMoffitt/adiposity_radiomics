...
<!-- set.seed(1234) -->
<!-- mstat <- maxstat.test(Surv(time = training_data$os_time_from_dx_Jan2025, -->
<!--              event = training_data$os_event_Jan2025) ~ BCI, -->
<!--         data =  training_data, -->
...
<!-- mstat <- maxstat.test(Surv(time = training_data$os_time_from_dx_Jan2025, -->
<!--              event = training_data$os_event_Jan2025) ~ BCI + age_at_diagnosis, -->
<!--         data =  training_data, -->
<!--                       smethod="LogRank", pmethod="exactGauss", -->
<!--                       abseps=0.01) -->
<!-- mstat -->
...
res.cut <- surv_cutpoint(training_data, time = "os_time_from_dx_Jan2025", event = "os_event_Jan2025",
   variables = c("BCI"))
...
```{r KM, fig.height=6}
myplot <- survfit(Surv(os_time_from_dx_Jan2025, event = os_event_Jan2025) ~ BCI_cat, data = training_data_cat)
...
```{r}
myplot <- survfit(Surv(rec_time_from_dx_Jan2025, event = rec_event_Jan2025) ~ BCI_cat, data = training_data_cat)
ggsurvplot(myplot, data = training_data_cat,
           title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
...
## OS
```{r HR OS}
coxph(Surv(os_time_from_dx_Jan2025, event = os_event_Jan2025) ~ 
...
## PFS
```{r}
coxph(Surv(rec_time_from_dx_Jan2025, event = rec_event_Jan2025) ~ 
        BCI_cat + age_at_diagnosis + tnm_cs_mixed_group_stage + debulking_status + site, 
      data = training_data_cat) %>%
  tbl_regression(exponentiate = TRUE) %>% 
  bold_p(t = .05) %>% 
...
myplot <- survfit(Surv(os_time_from_dx_Jan2025, event = os_event_Jan2025) ~ BCI_cat, data = validation_data_cat)
...
```{r}
myplot <- survfit(Surv(rec_time_from_dx_Jan2025, event = rec_event_Jan2025) ~ BCI_cat, data = validation_data_cat)
ggsurvplot(myplot, data = validation_data_cat,
           title = "PFS Analysis",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
...
<!-- myplot <- survfit(Surv(os_time_from_dx_Jan2025, event = os_event_Jan2025) ~ bmi_cat, data = training_data_cat) -->
<!-- ggsurvplot(myplot, data = training_data_cat, -->
<!--            title = "OS Analysis", -->
<!--            font.main = c(20, "bold", "black"), -->
<!--            font.x = c(18, "bold", "black"), -->
...
<!-- myplot <- survfit(Surv(os_time_from_dx_Jan2025, event = os_event_Jan2025) ~ bmi_cat, data = validation_data_cat) -->
<!-- ggsurvplot(myplot, data = training_data_cat, -->
<!--            title = "OS Analysis", -->
<!--            font.main = c(20, "bold", "black"), -->
<!--            font.x = c(18, "bold", "black"), -->
...
<!-- myplot <- survfit(Surv(os_time_from_dx_Jan2025, event = os_event_Jan2025) ~ bmi_cat, data = overall_data) -->
