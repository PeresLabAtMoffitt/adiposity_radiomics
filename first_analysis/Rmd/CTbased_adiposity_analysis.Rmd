...
Note: 
- I excluded the patient noted in "should exclude" variable.  
This exclude 2 out of the 3 patients with missing TNM.  
- debulking_status == "Complete" is combined with "Optimal"
- About the patient who was incorrectly classified as Upfront Surgery. On my side, I corrected the file for the Adiposity project, so we are good in our part.
...
adipose_data <- adipose_data %>% 
  mutate(excluded_complete_case = case_when(
    is.na(debulking_status)                   ~ "excluded",
    is.na(tnm_cs_mixed_group_stage)           ~ "excluded",
    TRUE                                      ~ "included"
    ))
...
tbl6 <- adipose_data %>% 
  ...
  bold_labels() %>% add_stat_label()
tbl_stack(list(tbl1, tbl2, tbl4, tbl5, tbl3, tbl6))
```

```{r}
adipose_data %>% 
   select(tert_imat_area_cm2, imat_area_cm2, treatment_type) %>% 
   tbl_summary(by = treatment_type,
             statistic=list(all_continuous() ~ "{min} - {max}"), 
             digits = all_continuous() ~ 1
             ) %>%  
...
adipose_data %>% 
  ...
  select(debulking_status, imp_debulking_status,
         tert_imat_area_cm2, 
         imp_bmi, imp_bmi_cat,
         bmi, bmi_cat,
         age_at_diagnosis,
...
adipose_data %>% 
  select(mrn, imp_bmi_cat,
         ...
         sat_area_cm2,
         total_fat_area) %>% 
  rename(c("SMI (skeletal muscle index)" = muscle_area_cm2_ind,
           "IMAT" = imat_area_cm2,
           "VAT" = vat_area_cm2,
           "SAT" = sat_area_cm2,
           "SMR (skeletal muscle radiodensity)" = muscle_mean_hu,
           "TAT" = total_fat_area)) %>% 
...
## Correlation
```{r}
a <- adipose_data %>% select(imp_bmi, bmi,
         muscle_area_cm2,
         imat_area_cm2,
         vat_area_cm2,
         sat_area_cm2,
...
mat1 <- mat %>% as_tibble() %>% 
  mutate(var = rownames(mat), .before=1) %>% 
  filter(str_detect(var, "bmi")) %>% 
  select(-c(contains("bmi")))
mat1
...
tbl1 <- adipose_data %>% select(pfs_event, pfs_time,
                                bmi_cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = adipose_data$pfs_time,
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ bmi_cat + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ imp_bmi_cat + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ tert_muscle_area_cm2_ind + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ tert_imat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ tert_vat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ tert_sat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ tert_total_fat_area + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
...
print("PH assumption no time capping strata on debulking")
coxph(Surv(time = adipose_data$pfs_time,
             event = adipose_data$pfs_event) ~ myosteatosis + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + strata(imp_debulking_status),
        data =  adipose_data)  %>% 
  cox.zph()
```
<br>
...
<!-- also adjusted by raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + imp_debulking_status -->
<!-- ```{r} -->
<!-- coxph(Surv(time = adipose_data$pfs_time, -->
<!--          event = adipose_data$pfs_event) ~ tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + imp_debulking_status, -->
<!--     data =  adipose_data) %>% -->
<!--   tbl_regression(exponentiate = TRUE,  -->
<!--                  include = c(tert_muscle_area_cm2, tert_imat_area_cm2, tert_vat_area_cm2, tert_sat_area_cm2)) %>% bold_p(t = .05) %>%  -->
<!--   add_nevent(location = "level") %>% add_n(location = "level") -->
<!-- ``` -->
<br>
<br>
...
#### Upfront Chemo
```{r}
tbl1 <- adipose_data_neo %>% select(pfs_event, pfs_time,
                                bmi_cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = adipose_data_neo$pfs_time,
...
#### Upfront Surgery
```{r}
tbl1 <- adipose_data_surg %>% select(pfs_event, pfs_time,
                                bmi_cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = adipose_data_surg$pfs_time,
...
<!-- ```{r} -->
<!-- coxph(Surv(time = adipose_data_surg$pfs_time, -->
<!--          event = adipose_data_surg$pfs_event) ~ tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 +  raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + imp_debulking_status, -->
<!--     data =  adipose_data_surg) %>% -->
<!--   tbl_regression(exponentiate = TRUE,  -->
<!--                  include = c(tert_muscle_area_cm2, tert_imat_area_cm2, tert_vat_area_cm2, tert_sat_area_cm2)) %>% -->
<!--   bold_p(t = .05) %>%  -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = adipose_data$os_time, -->
<!--          event = adipose_data$os_event) ~ tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + imp_debulking_status, -->
<!--     data =  adipose_data) %>% -->
<!--   tbl_regression(exponentiate = TRUE,  -->
<!--                  include = c(tert_muscle_area_cm2, tert_imat_area_cm2, tert_vat_area_cm2, tert_sat_area_cm2)) %>% -->
<!--   bold_p(t = .05) %>%  -->
...
<!-- model1 <- survreg(Surv(time = adipose_data$os_time, -->
<!--              event = adipose_data$os_event) ~  -->
<!--                tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + treatment_type + imp_debulking_status,  -->
<!--                   data = adipose_data, -->
<!--                   dist="weibull") -->
<!-- shapeParameter <- 1 / model1$scale -->
<!-- bind_cols(as_tibble(names(exp(-1 * shapeParameter * coef(model1)))), -->
...
```{r}
adipose_stage1_3 <- adipose_data_neo %>% 
  mutate(imp_tnm_cs_mixed_group_stage = case_when(
    imp_tnm_cs_mixed_group_stage == "I-II"        ~ "I-III",
    imp_tnm_cs_mixed_group_stage == "III"         ~ "I-III",
    TRUE                                          ~ as.character(imp_tnm_cs_mixed_group_stage)
  ))
```

```{r}
tbl1 <- adipose_stage1_3 %>% select(os_event, os_time,
                                bmi_cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = adipose_stage1_3$os_time,
                             event = adipose_stage1_3$os_event)),
                   exponentiate = TRUE) %>% 
  bold_labels() %>% italicize_levels() %>% 
  bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <-
  coxph(Surv(time = adipose_stage1_3$os_time,
             event = adipose_stage1_3$os_event) ~ 
          bmi_cat + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + imp_debulking_status,
        data =  adipose_stage1_3) %>%
  tbl_regression(exponentiate = TRUE, 
                 include = bmi_cat) %>% bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level")
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
...
print("AFT model")
model2 <- survreg(Surv(time = adipose_stage1_3$os_time,
             event = adipose_stage1_3$os_event) ~ 
               bmi_cat + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + imp_debulking_status, 
                  data = adipose_stage1_3,
                  dist="weibull")
...
<!--          event = adipose_data_neo$os_event) ~ tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + imp_debulking_status, -->
<!--     data =  adipose_data_neo) %>% -->
<!--   tbl_regression(exponentiate = TRUE,  -->
<!--                  include = c(tert_muscle_area_cm2, tert_imat_area_cm2, tert_vat_area_cm2, tert_sat_area_cm2)) %>% -->
<!--   bold_p(t = .05) %>%  -->
...
<!--                tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 + raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + imp_debulking_status,  -->
<!--                   data = adipose_data_neo, -->
<!--                   dist="weibull") -->
<!-- shapeParameter <- 1 / model2$scale -->
<!-- bind_cols(as_tibble(names(exp(-1 * shapeParameter * coef(model2)))), -->
...
<!-- ```{r} -->
<!-- coxph(Surv(time = adipose_data_surg$os_time, -->
<!--          event = adipose_data_surg$os_event) ~ tert_muscle_area_cm2 + tert_imat_area_cm2 + tert_vat_area_cm2 + tert_sat_area_cm2 +  raceeth1 + age_at_diagnosis + imp_tnm_cs_mixed_group_stage + imp_debulking_status, -->
<!--     data =  adipose_data_surg) %>% -->
<!--   tbl_regression(exponentiate = TRUE,  -->
<!--                  include = c(tert_muscle_area_cm2, tert_imat_area_cm2, tert_vat_area_cm2, tert_sat_area_cm2)) %>% -->
<!--   bold_p(t = .05) %>%  -->
...
#### PFS
```{r}
coxph(Surv(time = acsites_excluded$pfs_time,
...
```{r}
coxph(Surv(time = acsites_excluded_neo$pfs_time,
...
```{r}
coxph(Surv(time = acsites_excluded_surg$pfs_time,
...
adipose_data %>% 
  select(age_at_diagnosis, year_of_diagnosis, raceeth, raceeth1,
         ...
         vital_new, os_time, 
         has_the_patient_progressed, pfs_time,
         excluded_complete_case) %>% 
  
  tbl_summary(by= excluded_complete_case,
              type = list(year_of_diagnosis ~ "categorical", 
                          all_dichotomous() ~ "categorical"), 
              statistic=list(all_continuous() ~ "{mean} ({sd})"), 
              digits = all_continuous() ~ 1
...
#### PFS
```{r}
tbl1 <- comorbidities_yes %>% select(pfs_event, pfs_time,
                                bmi_cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = comorbidities_yes$pfs_time,
...
#### PFS
```{r}
tbl1 <- comorbidities_no %>% select(pfs_event, pfs_time,
                                bmi_cat) %>%
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = comorbidities_no$pfs_time,
